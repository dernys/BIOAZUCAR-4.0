...
export interface FieldPlot {
  ...
  createdAt?: string;
  updatedAt?: string;
  version?: string;                 // Optimistic lock & schema version
  // Governance & Sync
  dataClassification?: DataClassification;
  dataOrigin?: DataOrigin;
  dataQuality?: DataQuality;
...
/**
 * AgroWorkRequirement
 * Represents operational mechanization/field requirement generated by PDA
 * (Strict separation between Agricultural Need and CMMS Asset Allocation)
 */
export interface AgroWorkRequirement {
  id: string;
  tenantId: string;
  campaignId: string;
  plotId?: string;
...
/**
 * HarvestOrder
 * Official harvest requirement/order scheduled for a field plot
 */
export interface HarvestOrder {
  ...
  plotCode: string;
  varietyCode: string;
  targetHarvestTons: number;        // PLANNED by PDA
  scheduledHarvestDate: string;
  harvestFrontId?: string;
  status: "PLANNED" | "APPROVED" | "IN_EXECUTION" | "COMPLETED" | "CANCELLED";
  // Governance
  dataClassification?: DataClassification;
  dataOrigin?: DataOrigin;
  dataQuality?: DataQuality;
 ...
 * Real-time event recording physical cutting and transloading in the field
 */
export interface HarvestExecutionEvent {
  id: string;
  tenantId: string;
  campaignId: string;
  harvestOrderId: string;
  plotId: string;
  harvesterEquipmentId: string;
  transloaderEquipmentId?: string;
  operatorName?: string;
  executedDate: string;
  harvestedAreaHa: number;
  observedHarvestTons: number;       // OBSERVED field measurement
...
/**
 * CaneDispatch
 * CCT road transport dispatch ticket accompanying the cane truck to the factory
 */
export interface CaneDispatch {
  ...
  truckPlate: string;
  driverName?: string;
  origin: string;                    // UEB / Parcela
  dispatchTimestamp: string;
  estimatedNetTons: number;
  status: "DISPATCHED" | "IN_TRANSIT" | "ARRIVED" | "WEIGHED" | "CANCELLED";
  // Governance
  dataClassification?: DataClassification;
  dataOrigin?: DataOrigin;
  dataQuality?: DataQuality;
 ...
 * Independent industrial weighing & reception event at sugar mill scale
 */
export interface FactoryWeighingAndReception {
  id: string;                        // receptionId
  tenantId: string;
  campaignId: string;
  dispatchId: string;
  harvestOrderId: string;
  ...
  netWeightTons: number;             // gross - tare (WEIGHED / RECEIVED)
  labBrix?: number;
  labPol?: number;
  labPurity?: number;
  labFiber?: number;
  ...
  status: "RECEIVED" | "IN_YARD" | "SENT_TO_MILL" | "REJECTED";
  caneBatchId?: string;
  // Governance
  dataClassification?: DataClassification;
  dataOrigin?: DataOrigin;
...
/**
 * Operational Data Stages (ISA-95 Level 3/4)
 * Strict distinction between projection, measurement and processing
 */
...
/**
 * PlanVsActualVarianceItem
 * Rigorous variance comparison between planned target and executed reality
 */
export interface PlanVsActualVarianceItem {
  concept: string;
  category:
    | "AREAS"
    | "PREPARACION"
 ...
 * Complete closed-loop physical balance comparing PDA projections with mill actuals
 */
export interface ClosedLoopFeedbackSummary {
  tenantId: string;
  campaignId: string;
  campaignName: string;
  timestamp: string;
  // Physical flow milestones
  plannedCaneTons: number;           // PDA baseline projection
  actualHarvestedCaneTons: number;   // Field cutting observation
  factoryReceivedCaneTons: number;   // Weighbridge scale net weight
  processedCaneTons: number;         // Tandem milling execution
  sugarProductionTons: number;       // Factory recovery yield
  // Deviations (%)
  harvestVsPlanDeviationPercent: number;
  receptionVsHarvestDeviationPercent: number;
  millingVsReceptionDeviationPercent: number;

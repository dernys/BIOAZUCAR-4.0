...
export class TenantRuntime {
  ...
  public getTelemetrySnapshot(): TelemetryData {
    ...
    if (this.mode === "LIVE_OT") {
      ...
      const getValue = (key: keyof TelemetryData): number => {
      ...
      return {
        timestamp: new Date().toISOString(),
        tch: getValue("tch"),
        caneAccumToday: getValue("caneAccumToday"),
        caneBrix: getValue("caneBrix"),
        canePol: getValue("canePol"),
        canePurity: getValue("canePurity"),
        millingExtraction: getValue("millingExtraction"),
        imbibitionWaterFlow: getValue("imbibitionWaterFlow"),
        bagasseProductionRate: getValue("bagasseProductionRate"),
        bagasseBoilerConsumption: getValue("bagasseBoilerConsumption"),
        bagasseYardStorageRate: getValue("bagasseYardStorageRate"),
        bagasseMoisture: getValue("bagasseMoisture"),
        bagasseStockTotal: getValue("bagasseStockTotal"),
        boilerPressureHP: getValue("boilerPressureHP"),
        boilerTempHP: getValue("boilerTempHP"),
        steamFlowHP: getValue("steamFlowHP"),
        steamPressureLP: getValue("steamPressureLP"),
        steamTempLP: getValue("steamTempLP"),
        boilerEfficiency: getValue("boilerEfficiency"),
        flueGasO2: getValue("flueGasO2"),
        powerGeneratedMW: getValue("powerGeneratedMW"),
        powerInternalMW: getValue("powerInternalMW"),
        powerExportGridMW: getValue("powerExportGridMW"),
        gridFrequencyHz: getValue("gridFrequencyHz") || 0,
        powerFactor: getValue("powerFactor") || 0,
        gridVoltageKV: getValue("gridVoltageKV") || 0,
        clarifiedJuiceFlow: getValue("clarifiedJuiceFlow"),
        evaporatorSyrupBrix: getValue("evaporatorSyrupBrix"),
        sugarProductionTonsToday: getValue("sugarProductionTonsToday"),
        sugarBagsToday: getValue("sugarBagsToday"),
        factoryRecoveryYield: getValue("factoryRecoveryYield"),
        molassesProductionTons: getValue("molassesProductionTons"),
        oeeOverall: getValue("oeeOverall"),
        oeeAvailability: getValue("oeeAvailability"),
        oeePerformance: getValue("oeePerformance"),
        oeeQuality: getValue("oeeQuality"),
        mill3Vibration: getValue("mill3Vibration"),
        simulationScenario: "NORMAL",
        isSimulated: false,
        provenance: "OBSERVED_OT",
        source: `LIVE_OT (${this.otConfig.protocol}): ${this.otConfig.endpointUrl}`,
        quality: Object.keys(otLive).length > 0 ? "GOOD" : "UNCERTAIN",
      };
    }
